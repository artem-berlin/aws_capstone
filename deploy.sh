...
load_balancer_dns=$(terraform output -json | jq -r .load_balancer_dns.value)
echo $load_balancer_dns
export REACT_APP_BASE_URL="http://$(terraform output -json | jq -r .load_balancer_dns.value)"
#export REACT_APP_BASE_URL="http://$load_balancer-dns:8080"
